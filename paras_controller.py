...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
	    ...
	    # this rule drops the UDP traffic directed to h1 
            ...
            # this rule directs the HTTP packets to h4 to the controller
            match = parser.OFPMatch(eth_type = ether.ETH_TYPE_IP,
                                    ipv4_dst = '10.0.0.4',
            ...
            # this rule directs the HTTP packets to h2 to the controller
            match = parser.OFPMatch(eth_type = ether.ETH_TYPE_IP,
                                    ipv4_dst = '10.0.0.2',
                                    ip_proto = inet.IPPROTO_TCP,
                                    tcp_dst = 80)
    ...
    def handle_ip(self, datapath, in_port, pkt):
		...
		if (datapath.id == 1 or datapath.id == 3) and (tcp_pkt.dst_port == 80):
		eth_pkt = pkt.get_protocol(ethernet.ethernet)
		tcp_hd = tcp.tcp(ack=0, src_port = tcp_pkt.dst_port, dst_port = tcp_pkt.src_port, bits=0b001100)
